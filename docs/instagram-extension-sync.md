...
2. L’extension parcourt le flux sauvegardé du plus récent au premier post déjà connu.
3. Chaque image ou vidéo est envoyée directement vers R2 avec une URL PUT présignée.
4. Le serveur vérifie les objets R2, puis crée ou met à jour le post dans PostgreSQL.
5. Un post déjà présent est mis à jour ; il n’est jamais dupliqué.
...
1. Décompresser `outputs/insta-saved-sync-v4.0.0.zip` dans un dossier permanent.
...
La première synchronisation utilise les identifiants déjà présents dans la DB pour
amorcer l’index incrémental. Si les anciens imports ne possèdent aucun `external_id`,
le premier passage peut reparcourir l’historique ; les upserts empêchent néanmoins
les doublons.
